...
const EmptyData = pageId => ({
  ...
  allPages: [
    {
      id: 1,
      title: `Notion 데이터를 가져오지 못했습니다. NOTION_PAGE_ID 를 확인하세요 (현재: ${pageId})`,
      summary:
        '설정 방법: https://docs.tangly1024.com/article/vercel-deploy-notion-next',
      status: 'Published',
      type: 'Post',
      slug: 'oops',
...
function getSiteInfo({ collection, block, rawMetadata, NOTION_CONFIG }) {
  ...
  const defaultPageCover =
    NOTION_CONFIG?.HOME_BANNER_IMAGE || BLOG.HOME_BANNER_IMAGE || '/bg_image.jpg'
  ...
  // NOTION_CONFIG / 환경변수로 사이트명을 명시했으면 노션 DB 이름보다 우선한다
  const title =
    NOTION_CONFIG?.TITLE || BLOG.TITLE || collection?.name?.[0][0] || defaultTitle
